...
dump(model, 'D:/Escritorio/Corpoica/Canopy Attributes/AdaBoost.joblib')
    
#######################
# CANOPY ATTRIBUTES
#######################
#model = load('D:/Escritorio/Corpoica/Canopy Attributes/AdaBoost.joblib')
output = open(root_out + 'Canopy Attributes_AdaBoost.csv', 'w')
output.write('image; gap_fraction; large_gap_fraction; foliage cover; '
             'canopy cover; crown porosity; clumping index; LAI\n')
L = os.listdir(root_in)     
for l in range(len(L)):
    file = L[l]
    file_split = os.path.splitext(file)
    if file_split[1] == '.jpg':
        ...
        imageio.imsave(root_out + file_split[0] + '_AdaBoost_class.tif',classify)
        ...
        canopy_cover = 1 - large_gap_fraction
